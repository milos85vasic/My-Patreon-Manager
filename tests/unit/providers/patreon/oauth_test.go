...
func newTestOAuth2Manager(t *testing.T, serverURL string) *patreon.OAuth2Manager {
	t.Helper()
	manager := patreon.NewOAuth2Manager("test-client-id", "test-client-secret", "initial-access", "initial-refresh")
...
func TestOAuth2Manager_GetAccessToken(t *testing.T) {
	manager := patreon.NewOAuth2Manager("cid", "secret", "my-token", "refresh")
	assert.Equal(t, "my-token", manager.GetAccessToken())
}

func TestOAuth2Manager_ClearCredentials(t *testing.T) {
	manager := patreon.NewOAuth2Manager("cid", "secret", "access", "refresh")
...
func TestOAuth2Manager_Refresh_NoRefreshToken(t *testing.T) {
	manager := patreon.NewOAuth2Manager("cid", "secret", "access", "")
	err := manager.Refresh(context.Background())
	assert.Error(t, err)
	assert.True(t, errors.IsInvalidCredentials(err))
}
